...
import com.moko.support.lw004.handler.MokoCharacteristicHandler;
...
public class LoRaLW004MokoSupport extends MokoBleLib {
    private HashMap<OrderCHAR, BluetoothGattCharacteristic> mCharacteristicMap;

    private static volatile LoRaLW004MokoSupport INSTANCE;

    private Context mContext;

    private MokoBleConfig mBleConfig;

    private LoRaLW004MokoSupport() {
        //no instance
    }

    public static LoRaLW004MokoSupport getInstance() {
        if (INSTANCE == null) {
            synchronized (LoRaLW004MokoSupport.class) {
                if (INSTANCE == null) {
                    INSTANCE = new LoRaLW004MokoSupport();
                }
            }
        }
        return INSTANCE;
