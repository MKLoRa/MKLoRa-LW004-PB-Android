# LW004 Android SDK Guide（English）
...
**`com.moko.support.lw004.LoRaLW004MokoSupport`**
...
**`com.moko.support.lw004.OrderTaskAssembler`**

We assemble read data and write data to `OrderTask`, send the task to the device through `LoRaLW004MokoSupport `, and receive the resopnse.
...
First of all, you should initialize the LoRaLW004MokoSupport.We recommend putting it in Application.
...
Before operating the Bluetooth scanning device, we need to apply for permission, which we have added in LoRaLW004MokoSupport `AndroidManifest.xml`
...
```
LoRaLW004MokoSupport.getInstance().connDevice(beaconXInfo.mac);
```
...
```
LoRaLW004MokoSupport.getInstance().sendOrder(OrderTaskAssembler.setPassword(password));
...
```
// read lora region
LoRaLW004MokoSupport.getInstance().sendOrder(derTaskAssembler.getLoraRegion());
...
// get result
@Subscribe(threadMode = ThreadMode.MAIN)
public void onOrderTaskResponseEvent(OrderTaskResponseEvent event) {
...
2.`EventBus` is used in the SDK and can be modified in `LoRaLW004MokoSupport` if you want to use other communication methods.
